...
THROTTLE_STEP_UP = 0.04
THROTTLE_STEP_DOWN = 0.1
    ...
    try:
        ...
        while True:
          try:
            ...
            if person_dets:
                # 1. If we already have a locked target ID, try to keep following it
                ...
                # 2. If locked ID not found, keep waiting up to 3 sec before reacquiring
                if target_det is None and active_target_id is not None:
                    if (now - active_target_last_seen) > target_persistent_seconds:
                        active_target_id = None

                # 3. If no active target now, acquire the closest visible one
            ...
            # briefly search in the last known direction, then stop and clear the lock.
                ...
                if active_target_id is not None and (now - active_target_last_seen) <= target_persistent_seconds and last_target_x is not None and last_target_time is not None and (now - last_target_time) <= target_persistent_seconds:
